...
class Ppal:
	...
	def estimader(self,preprocesado,model,coef):
		dec=3 #Cantidad de decimales de redondeo
		return round(np.dot(model,preprocesado)+coef, dec)
	...
	def button_measure(self):
		...
		Prom=np.append(Prom,values[0])
		if debug:
			print "Prom inicializado:"
			print Prom
		j=1
		for i in range(1,len(wvl_int)):
		...
		self.desplegar_mensaje("Medicion no."+str(self.i)+" realizada. Resultado PLS="+str(pls)+". Estado="+str(signo_pls)+"("+Estado+")")
